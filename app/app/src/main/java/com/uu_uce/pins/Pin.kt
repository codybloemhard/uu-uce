...
class Pin(
    ...
    private val imageHeight =
        pinSize * (image.intrinsicHeight.toFloat() / image.intrinsicWidth.toFloat())
    ...
    fun draw(viewport: Pair<p2, p2>, view: View, canvas: Canvas) {
        ...
        pinBbMin =
            screenToCoord(Pair(minX.toFloat(), maxY.toFloat()), viewport, view.width, view.height)
        pinBbMax =
            screenToCoord(Pair(maxX.toFloat(), minY.toFloat()), viewport, view.width, view.height)

        if (!aaBoundingBoxContains(
                viewport.first,
                viewport.second,
                p2(pinBbMin.east, pinBbMin.north),
                p2(pinBbMax.east, pinBbMax.north)
            )
        ) {
